...
train_transform = A.Compose(
    [
        A.Resize(height=config['dataset']['image_height'], width=config['dataset']['image_width']),
        A.CLAHE(clip_limit=4.0, tile_grid_size=(4, 4), p=1.0),
        A.Rotate(limit=35, p=0.5),
        A.HorizontalFlip(p=0.5),
        A.VerticalFlip(p=0.3),
        A.Normalize(mean=config['dataset']['mean'], std=config['dataset']['std'], max_pixel_value=255.0),
        ToTensorV2(),
    ]
)
